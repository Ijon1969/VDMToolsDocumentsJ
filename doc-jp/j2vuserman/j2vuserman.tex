...
これを行うために、最初に \ToolboxName{} をスタートし(これを行う方法の詳細は VDM++ Toolboxの一般向けユーザーマニュアル \cite{UserManPP-SCSK}で見ることができる)、次に (\guicmd{Project Operations})ツールバー上の\raisebox{-0.4mm}{\includegraphics[width=0.03\textwidth]{plus}}  (\guicmd{Add Files}) ボタンを押す (あるいは、 \guicmd{Project} メニュー上でアクション \guicmd{Add File to Project}を選択することもできる)。 
